...
main_directories_names = ['Water_nozzleCirc', 'Water_nozzleQuad', 'Water_nozzleTri', 'Water_nozzleCurv']

for item in external_diameters:
    main_directory_path = os.path.join(home_directory_name,main_directories_names[item])
    im = im.loop_through_dir(main_directory_path, external_diameters[item])
